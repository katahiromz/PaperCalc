...
class AlgoBase {
  ...
  getMapDigit(ix, iy) {
    if (!this.mapping.has(iy)) return undefined;
  ...
  addMissingZero(iy, test = false) {
    ...
    if (iDot === undefined) return false; // 小数点がなければ何もしない
  ...
  fixTrailingZeros(iy, test = false) {
    let ix0 = this.min_x(iy), ix1 = this.max_x(iy);
    let iDot = this.getMapDot(iy);
    if (iDot === undefined) return false;
  ...
  readRowNumber(iy, ignoreDot = false) {
    ...
    if (ix0 === undefined && iDot === undefined) return "";

    let start = (iDot !== undefined) ? Math.min(ix0 ?? iDot, iDot) : ix0;
    let end = (iDot !== undefined) ? Math.max(ix1 ?? iDot, iDot) : ix1;
